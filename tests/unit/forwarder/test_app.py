...
class TestForwarderApp:
    ...
    def test_get_app_config_not_initialized(self):
        ...
        with patch("webhook_relay.forwarder.app._app_config", None):
            with pytest.raises(RuntimeError, match="Application config not initialized"):
                get_app_config()
    ...
    def test_load_config_from_file(self, tmp_path, forwarder_config):
        ...
        assert loaded_config.gcp_config.project_id == forwarder_config.gcp_config.project_id
    ...
    def test_setup_app(self, forwarder_config, mock_queue_client):
        ...
        with patch("webhook_relay.forwarder.app.create_queue_client") as mock_create_client, \
             patch("webhook_relay.forwarder.app.WebhookForwarder") as mock_forwarder_class, \
             patch("webhook_relay.forwarder.app.logger"):
    ...
    async def test_run_forwarder(self, forwarder_config):
        ...
        with patch("webhook_relay.forwarder.app.start_metrics_server") as mock_start_metrics, \
             patch("webhook_relay.forwarder.app.metrics.up") as mock_up, \
             patch("webhook_relay.forwarder.app.logger"):
            
            # Set up the mock for _app_config, _forwarder, and _shutdown_event
            with patch("webhook_relay.forwarder.app._app_config", forwarder_config), \
                 patch("webhook_relay.forwarder.app._forwarder", mock_forwarder), \
                 patch("webhook_relay.forwarder.app._shutdown_event", mock_event):
                ...
                mock_start_metrics.assert_called_once_with(
                    forwarder_config.metrics.port,
                    forwarder_config.metrics.host
                )
    ...
    def test_cli_serve_command(self, forwarder_config, tmp_path):
        ...
        with patch("webhook_relay.forwarder.app.setup_app") as mock_setup, \
             patch("webhook_relay.forwarder.app.signal.signal") as mock_signal, \
             patch("webhook_relay.forwarder.app.asyncio.run") as mock_run, \
             patch("webhook_relay.forwarder.app.logger"):
