...
class TestWebhookRoutes:
    ...
    def test_receive_webhook_invalid_signature(self, collector_client, collector_config):
        """Test that receiving a webhook with an invalid signature returns a 401."""
        # Get the GitHub source from the config
        github_source = next(src for src in collector_config.webhook_sources if src.name == "github")
    ...
    def test_receive_webhook_valid_signature(self, collector_client, collector_config, mock_queue_client):
