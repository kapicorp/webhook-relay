...
class TestCollectorServer:
    ...
    def test_metrics_setup(self, collector_config):
        ...
        with patch(
            "webhook_relay.collector.server.start_metrics_server"
        ) as mock_start_metrics, patch(
            "webhook_relay.collector.server.metrics.up"
        ) as mock_up:
            ...
            from webhook_relay.collector.server import (
                start_metrics_server as original_start_metrics,
            )

            # Call the original metrics startup with the config
            if collector_config.metrics.enabled:
                original_start_metrics(
                    collector_config.metrics.port, collector_config.metrics.host
                )
